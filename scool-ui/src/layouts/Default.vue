...
import {vebSocketMixin} from "../components/mixins/vebSocketMixin";
  ...
  window.addEventListener("scroll", function () {
    ...
    if (document.body.contains(document.getElementById("navConverter"))) {
      ...
      if (lastScrollTop > (getOffset(document.getElementById('navConverter')).top - 60)) {
        removeClass(document.querySelector('.nav_bar'), 'navbar--extended');
      } else {
        addNewClass(document.querySelector('.nav_bar'), 'navbar--extended');
      }
    }
...
export default {
  ...
  mixins: [vebSocketMixin],

  methods: {
    async connect() {
      if (this.$store.getters.getIsAuthenticated) {
        this.notificationSocket = new WebSocket(
            this.$store.state.webSocketUrl + '/ws/notifications/' + this.$store.state.authUser.id + '/'
        );
